...
(function () {
  ...
  auth.onAuthStateChanged(async (u) => {
    ...
    if (topAvatarChat && u.photoURL) {
      topAvatarChat.src = u.photoURL;
    }
    await loadUsers();
    listenPublic();
  });
  ...
  async function loadUsers() {
    ...
    snap.forEach((doc) => {
      ...
      div.innerHTML = `<img src="${
        d.profileImage || "../assets/placeholder-avatar.svg"
  ...
  function appendMessage(m, isSelf) {
    ...
    const postedAt =
      m.timestamp && m.timestamp.toDate ? m.timestamp.toDate().toLocaleTimeString() : "";
    header.textContent = (m.name || m.uid || "User") + (postedAt ? " - " + postedAt : "");
